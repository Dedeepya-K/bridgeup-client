                ...
                <div className="bg-white rounded-xl shadow p-5 space-y-3">
                  ...
                  {weeklyReport && (
                    <div className="space-y-3">
                      ...
                      <button onClick={() => {
  const content = `BridgeUp Weekly Report — ${weeklyReport.teacherName}
Generated: ${new Date(weeklyReport.generatedAt).toLocaleString('en-AU')}

SUMMARY
${weeklyReport.summary}

METRICS
- Messages Sent: ${weeklyReport.totalMessages}
- Parent Replies: ${weeklyReport.totalReplies}
- Activities Tried: ${weeklyReport.tried}
- Families Struggling: ${weeklyReport.struggled}
- Languages Reached: ${Object.keys(weeklyReport.languages).length}

SUBJECTS COVERED
${Object.entries(weeklyReport.subjectBreakdown).map(([s,c]) => `• ${s}: ${c} update${c>1?'s':''}`).join('\n')}

SENTIMENT BREAKDOWN
- Positive replies: ${weeklyReport.sentiments?.positive || 0}
- Questions: ${weeklyReport.sentiments?.question || 0}
- Concerns: ${weeklyReport.sentiments?.concern || 0}

Generated by BridgeUp — Powered by CurricuLLM
Australian Privacy Act 1988 Compliant`
  ...
  a.href = url
  a.download = `BridgeUp_Weekly_Report_${new Date().toISOString().split('T')[0]}.txt`
  a.click()
